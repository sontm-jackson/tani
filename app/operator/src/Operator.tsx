...
export default function Operator() {
  ...
  useEffect(() => {
    load().catch((e) => setNotice({ ok: false, msg: e.message })).finally(() => setLoading(false));
  ...
  if (!op)
    return (
      <div className="wrap page">
        <div className="notice notice-err">No operator. Run <b>npm run seed</b> in app/api first.</div>
      </div>
    );

  return (
    <div className="wrap page">
      <div className="eyebrow">Cooperative dashboard</div>
      <h1 className="title">{op.name}</h1>
      <p className="sub">{op.region}</p>
      ...
      <div className="row">
        <div className="card hero" style={{ flex: 2 }}>
          <div>
            ...
            <div className="meta">
              <a className="link" href={op.poolExplorer} target="_blank" rel="noreferrer">view pool account on-chain ↗</a>
            </div>
          </div>
        ...
        <div className="card stat">
          ...
          <div className="v">{op.counts.farmers}</div>
        </div>
        <AnchorCard anchor={anchor} />
      </div>

      {/* arrivals — QR shipment verification */}
      <Arrivals onChanged={load} onNotice={(m) => setNotice({ ok: true, msg: m })} />
      ...
      <div className="section">
        ...
        {panel === "lot" && (
          <NewLotForm farmers={farmers} busy={busy === "lot"}
        ...
        <div className="card">
          ...
          {lots.map((l) => (
            <div className="lot" key={l.id} style={{ borderBottom: "1px solid var(--green-tint)" }}>
              <div>
                <div className="code">
                  {l.code} <span className={`pill commodity-${l.commodity}`} style={{ marginLeft: 6 }}>{l.commodity}</span>
                </div>
            ...
            <div className="disb" key={d.id}>
              ...
              <div className="payments">
                {d.payments.map((p: any, i: number) => (
                  <div className="pay" key={i}><span>{p.farmerName}</span><b>{fmtUsdc(p.amount)}</b></div>
                ))}
              </div>
            </div>
          ))}
      ...
      <div className="section">
        <div style={{ display: "flex", alignItems: "center" }}>
          <h2 style={{ flex: 1 }}>Farmer roster</h2>
...
function FundBox({ busy, onFund }: { busy: boolean; onFund: (n: number) => void }) {
  ...
  return (
    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
      ...
      <button className="btn-ghost" disabled={busy || !Number(amt)} onClick={() => onFund(Number(amt))}>
...
function AnchorCard({ anchor }: { anchor: any }) {
  return (
    <div className="card stat" style={{ minWidth: 230 }}>
      ...
      {!anchor ? (
        <div className="muted" style={{ fontSize: 13, marginTop: 6 }}>checking…</div>
      ) : (
        <>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 4 }}>
            <span className={`pill ${anchor.reachable ? "pill-success" : "pill-failed"}`}>
              {anchor.reachable ? "live" : "offline"}
            </span>
...
function NewLotForm({ farmers, busy, onSubmit }: any) {
  ...
  const contributions = Object.entries(rows)
    .filter(([, v]) => Number(v) > 0)
    .map(([farmerId, v]) => ({ farmerId, quantityKg: Number(v) }));
  return (
    <div className="card pad form-panel">
      <div className="row">
        <div className="field" style={{ flex: 2 }}>
          <label>Lot code</label>
          <input value={code} onChange={(e) => setCode(e.target.value)} placeholder="LOT-2026-002" />
        </div>
        <div className="field">
          <label>Commodity</label>
          <input value={commodity} onChange={(e) => setCommodity(e.target.value)} placeholder="coffee" />
        </div>
      </div>
      ...
      <div className="contrib-grid">
        {farmers.map((f: any) => (
          <div key={f.id} className="contrib-row">
            <span>{f.name}</span>
            <input type="number" placeholder="0" value={rows[f.id] ?? ""}
              onChange={(e) => setRows({ ...rows, [f.id]: e.target.value })} style={{ width: 80 }} />
          </div>
        ))}
      </div>
...
function NewRuleForm({ busy, onSubmit }: any) {
  ...
  return (
    <div className="card pad form-panel">
      <div className="row">
        <div className="field" style={{ flex: 2 }}><label>Rule name</label>
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Coffee verified payout" /></div>
        <div className="field"><label>Commodity</label>
          <input value={commodity} onChange={(e) => setCommodity(e.target.value)} /></div>
        <div className="field"><label>USDC / kg</label>
          <input type="number" value={ratePerKg} onChange={(e) => setRate(e.target.value)} /></div>
      </div>
...
function NewFarmerForm({ busy, onSubmit }: any) {
  ...
  return (
    <div className="card pad form-panel">
      ...
      <div className="muted" style={{ fontSize: 12.5, marginBottom: 10 }}>Provisions a custodial Stellar wallet on testnet (~5s).</div>
      <button className="btn-green" disabled={busy || !name || !phone}
        onClick={() => onSubmit({ name, phone, village })}>
        {busy ? "Provisioning…" : "Add farmer"}
      </button>
    </div>
  );
}
